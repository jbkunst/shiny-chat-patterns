...
app_ui = ui.page_sidebar(
        ...
        ui.accordion(
            ui.accordion_panel(
                ...
                ui.input_select(
                    "continente",
                    "Continente",
                    choices=sorted(paises["continente"].unique()),
                    multiple=True,
                ),
                ui.input_select(
                    "pais",
                    "País",
                    choices=paises["pais"].tolist(),
                    multiple=True,
                ),
                ui.input_slider(
                    "pib",
                    "PIB per cápita",
                    min=float(paises["pib_per_capita"].min()),
                    max=float(paises["pib_per_capita"].max()),
                    value=(
                        float(paises["pib_per_capita"].min()),
                        float(paises["pib_per_capita"].max()),
                    ),
                ),
                ui.input_slider(
                    "esperanza",
                    "Esperanza de vida",
                    min=int(paises["esperanza_de_vida"].min()),
                    max=int(paises["esperanza_de_vida"].max() + 1),
                    value=(
                        float(paises["esperanza_de_vida"].min()),
                        float(paises["esperanza_de_vida"].max()),
                    ),
                ),
                ui.input_slider(
                    "poblacion",
                    "Población (millones)",
                    min=0,
                    max=int(paises["poblacion"].max() / 1e6 + 1),
                    value=(0, int(paises["poblacion"].max() / 1e6 + 1)),
                ),
            ),
            ui.accordion_panel(
                "Consulta con chat",
                ui.chat_ui(
                    "chat",
                    messages=[saludo],
                    placeholder="Describe el filtro...",
                ),
            ),
    ...
    ui.layout_columns(
        ...
        ui.value_box(
            "Población",
            ui.output_text("poblacion_total"),
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def data_inputs():
        frame = paises
        if input.continente():
            frame = frame[frame["continente"].isin(input.continente())]
        if input.pais():
            frame = frame[frame["pais"].isin(input.pais())]
        frame = frame[frame["pib_per_capita"].between(*input.pib())]
        frame = frame[frame["esperanza_de_vida"].between(*input.esperanza())]
        return frame[frame["poblacion"].between(*(valor * 1e6 for valor in input.poblacion()))]
    ...
    def _():
        modo.set("inputs")
        titulo_actual.set("Filtros tradicionales")
    ...
    async def actualizar_dashboard(
        ...
        return {"mensaje": "Dashboard actualizado mediante query.", "registros": len(resultado)}
